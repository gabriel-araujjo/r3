...
START_TEST(test_pattern_to_opcode) {
  ck_assert(r3_pattern_to_opcode("\\w+", strlen("\\w+")) ==
            OP_EXPECT_MORE_WORDS);
  ck_assert(r3_pattern_to_opcode("\\d+", strlen("\\d+")) ==
            OP_EXPECT_MORE_DIGITS);
  ck_assert(r3_pattern_to_opcode("[^/]+", strlen("[^/]+")) ==
            OP_EXPECT_NOSLASH);
...
START_TEST(test_r3_slug_compile) {
  ...
  ck_assert_str_eq(c = r3_slug_compile(path2, strlen(path2)),
                   "^/what/([^/]+)-foo");
  zfree(c);
...
START_TEST(test_r3_slug_find_placeholder_with_broken_slug) {
  ...
  const char *slug =
      r3_slug_find_placeholder(sl_test, strlen(sl_test), &slug_len);
  ck_assert(slug == 0);
}
END_TEST
