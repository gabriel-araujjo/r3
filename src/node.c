 ...
 */
R3Edge *r3_node_connectl(R3Node *n, const char *pat, int len, int dupl,
                         R3Node *child) {
 ...
 */
R3Edge *r3_node_find_edge(const R3Node *n, const char *pat,
...
/**
 * This function combines ['/foo', '/bar', '/{slug}'] into
 * (/foo)|(/bar)|/([^/]+)}
 *
...
int r3_tree_compile_patterns(R3Node *n, char **errstr) {
  ...
  for (; i < n->edges.size; i++) {
    ...
    if (e->has_slug) {
      ...
      size_t slug_pat_len = strlen(slug_pat);
      info("slug_pat for pattern: %s\n", slug_pat);
      memcpy(p, slug_pat, slug_pat_len);
      p += slug_pat_len;
      zfree(slug_pat);
    } else {
      memcpy(p, "^(", 2);
      ...
      memcpy(p, e->pattern.base, e->pattern.len);
      p += e->pattern.len;
  ...
  *p = '\0';
  ...
  n->pcre_pattern = pcre_compile(n->combined_pattern, /* the pattern */
  ...
  if (!n->pcre_extra) {
    if (errstr) {
      asprintf(errstr, "PCRE study failed at offset %s, pattern: %s",
 ...
 *
 * r3_tree_matchl returns NULL when the path does not match. returns *node when
 * the path matches.
 *
 * @param node         n        the root of the tree
 * @param char*        path     the URL path to dispatch
 * @param int          path_len the length of the URL path.
 * @param match_entry* entry match_entry is used for saving the captured dynamic
 * strings from pcre result.
 */
R3Node *r3_tree_matchl(const R3Node *n, const char *path, unsigned int path_len,
                       match_entry *entry) {
  info("try matching: %s\n", path);
  ...
  if (n->pcre_pattern) {
    ...
    rc = pcre_exec(n->pcre_pattern,     /* the compiled pattern */
                   n->pcre_extra, path, /* the subject string */
    ...
    restlen = path_len -
              ov[1]; // if it's fully matched to the end (rest string length)
    int *inv = ov + 2;
    if (!restlen) {
...
inline R3Edge *r3_node_find_edge_str(const R3Node *n, const char *str,
                                     int str_len) {
  R3Edge *e;
...
static r3_iovec_t *router_append_slug(R3Route *route, const char *slug,
                                      unsigned int len) {
  r3_iovec_t *temp;
...
static void get_slugs(R3Route *route, const char *path, int path_len) {
  ...
  while (plh < (path + path_len)) {
    ...
    if ((plh + l) >= (path + path_len))
      break;
  }
}

R3Route *r3_node_append_route(R3Node *tree, const char *path, int path_len,
                              int method, void *data) {
 ...
 */
R3Route *r3_tree_insert_routel_ex(R3Node *tree, int method, const char *path,
                                  int path_len, void *data, char **errstr) {
  R3Node *ret =
      r3_tree_insert_pathl_ex(tree, path, path_len, method, 1, data, errstr);
 ...
 *
 * 1.  "/foo/{slug}" vs "/foo/bar"                      => common prefix =
 * "/foo/"
 * 2.  "{slug}/hate" vs "{slug}/bar"                    => common prefix =
 * "{slug}/"
 ...
 */
R3Edge *r3_node_find_common_prefix(R3Node *n, const char *path, int path_len,
                                   int *prefix_len, char **errstr) {
  unsigned int i = 0;
  int prefix = 0;
  *prefix_len = 0;
  R3Edge *e = NULL;
  for (i = 0; i < n->edges.size; i++) {
    ...
    prefix = strndiff((char *)path, n->edges.entries[i].pattern.base,
                      n->edges.entries[i].pattern.len);
 ...
 */
R3Node *r3_tree_insert_pathl_ex(R3Node *tree, const char *path,
                                unsigned int path_len, int method,
                                unsigned int router, void *data,
                                char **errstr) {
  R3Node *n = tree;
  ...
  if (!prefix_len) {
    ...
    if (slug_cnt > 1) {
      ...
      if (p) {
        p = r3_slug_find_placeholder(p + slug_len + 1, path_len - slug_len - 1,
                                     NULL);
      }
#ifdef DEBUG
      assert(p);
      ...
      return r3_tree_insert_pathl_ex(child, p, path_len - (int)(p - path),
                                     method, 1, data, errstr);
      ...
      if (slug_cnt == 1) {
        ...
        const char *slug_p =
            r3_slug_find_placeholder(path, path_len, &slug_len);
        unsigned int slug_pattern_len = 0;
        const char *slug_pattern =
            r3_slug_find_pattern(slug_p, slug_len, &slug_pattern_len);
        ...
        if (restlen) {
          return r3_tree_insert_pathl_ex(c2, slug_p + slug_len, restlen, method,
                                         1, data, errstr);
        }
      ...
      return child;
             ...
             e->pattern.len) { // fully-equal to the pattern of the edge

    // there are something more we can insert
    if (subpath_len > 0) {
      return r3_tree_insert_pathl_ex(e->child, subpath, subpath_len, method, 1,
                                     data, errstr);
    } else {
      // there are no more path to insert

      // see if there is an endpoint already, we should n't overwrite the data
      // on child. but we still need to append the route.
...
inline int r3_route_cmp(const R3Route *r1, const match_entry *r2) {
  ...
  if (r1->remote_addr_pattern.len && r2->remote_addr.len) {
    ...
    */
    if (strncmp(r1->remote_addr_pattern.base, r2->remote_addr.base,
                r2->remote_addr.len)) {
      return -1;
    }
  }
