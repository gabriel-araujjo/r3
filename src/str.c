...
int r3_pattern_to_opcode(const char *pattern, unsigned int len) {
  ...
  if (strncmp(pattern, "[0-9a-z]+", len) == 0 ||
      strncmp(pattern, "[a-z0-9]+", len) == 0) {
    return OP_EXPECT_MORE_WORDS;
  }
...
char *r3_inside_slug(const char *needle, int needle_len, char *offset,
                     char **errstr) {
  char *s1 = offset;
  char *s2 = offset;
...
const char *r3_slug_find_placeholder(const char *s1, unsigned int str_len,
                                     unsigned int *len) {
  const char *c;
  const char *s2;
  int cnt = 0;
 ...
 */
const char *r3_slug_find_pattern(const char *s1, unsigned int str_len,
                                 unsigned int *len) {
  const char *c;
  const char *s2;
  unsigned int cnt = 1;
 ...
 */
const char *r3_slug_find_name(const char *s1, unsigned int str_len,
                              unsigned int *len) {
  const char *c;
  const char *s2;
...
char *r3_slug_compile(const char *str, unsigned int len) {
  ...
  *o = '^';
  ...
  memcpy(o, str, s1 - str); // string before slug
  o += (s1 - str);
  ...
  s1 += s1_len;
  memcpy(o, s1, len - (s1 - str)); // string after slug
  o += len - (s1 - str);
  *o = '\0';
  return out;
}
